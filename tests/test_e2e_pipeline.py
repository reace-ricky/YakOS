...
class TestE2EPipeline:
    ...
    def test_feedback_loop_round_trip(self, tmp_path):
        """Pipeline → generate actuals → feed back into calibration."""
        pool = _make_dk_pool(8)
        pool = _run_projections(pool)
        pool = _run_enrich(pool)

        # Simulate actuals (projection + noise)
        np.random.seed(123)
        actuals = pd.DataFrame({
            "player_name": pool["player_name"].values,
            "actual_fp": pool["proj"].values + np.random.randn(len(pool)) * 4.0,
        })

        from yak_core.result_recorder import record_contest_results

        with mock.patch("yak_core.calibration_feedback._FEEDBACK_DIR", str(tmp_path / "cal")), \
             mock.patch("yak_core.calibration_feedback._HISTORY_FILE", str(tmp_path / "cal" / "h.json")), \
             mock.patch("yak_core.calibration_feedback._CORRECTIONS_FILE", str(tmp_path / "cal" / "c.json")), \
             mock.patch("yak_core.edge_feedback._FEEDBACK_DIR", str(tmp_path / "edge")), \
             mock.patch("yak_core.edge_feedback._SIGNAL_FILE", str(tmp_path / "edge" / "s.json")), \
             mock.patch("yak_core.edge_feedback._WEIGHTS_FILE", str(tmp_path / "edge" / "w.json")):
            ...
            assert summary["players_matched"] > 0
            assert summary["calibration"] is not None
            assert "error" not in summary.get("calibration", {})

            # Now apply corrections to a fresh pool
            ...
            if corrections.get("n_slates", 0) > 0:
                corrected = apply_corrections(pool, corrections)
                assert "proj_pre_correction" in corrected.columns
                assert "proj_correction" in corrected.columns
                # Corrections should be non-zero for some rows
                n_corrected = (corrected["proj_correction"].abs() > 0.01).sum()
                assert n_corrected > 0, "At least some players should have corrections"

    def test_signal_weight_evolution(self, tmp_path):
        """After recording results, signal weights should diverge from defaults."""
        pool = _make_dk_pool(8)
        pool = _run_projections(pool)
        pool = _run_enrich(pool)

        edge_df = _run_edge_metrics(pool)
        for col in ["smash_prob", "bust_prob", "leverage"]:
            if col in edge_df.columns and col not in pool.columns:
                pool = pool.merge(edge_df[["player_name", col]], on="player_name", how="left")

        # Record 3 slates with different noise profiles
        from yak_core.result_recorder import record_contest_results
        ...
        with mock.patch("yak_core.calibration_feedback._FEEDBACK_DIR", str(tmp_path / "cal")), \
            ...
            for i, date in enumerate(["2026-03-05", "2026-03-06", "2026-03-07"]):
                np.random.seed(i * 10)
                actuals = pd.DataFrame({
                    "player_name": pool["player_name"].values,
                    "actual_fp": pool["proj"].values + np.random.randn(len(pool)) * (3.0 + i),
                })
                record_contest_results(date, pool, actuals, "GPP")

            # Check that signal_weights.json was created
            wf = tmp_path / "edge" / "w.json"
            assert wf.exists(), "Signal weights file should exist after 3 slates"

            with open(wf) as f:
                data = json.load(f)
            assert data.get("n_slates", 0) >= 3
