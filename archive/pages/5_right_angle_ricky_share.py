"""Right Angle Ricky — shared read-only page (friend-facing).
...
Star/select players from Edge Analysis → auto-Lock in Optimizer;
...
# ── Session-state key prefix (unique to shared page) ─────────────────
_K = "_ricky_share_"
...
def _render_player_star_select(
    ...
    Updates ``_ricky_share_starred_players`` and ``_ricky_share_fade_players``
    in session state.
    """
