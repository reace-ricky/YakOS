...
def _run_nba_backfill(api_key: str, start_date: date, end_date: date) -> None:
    ...
    for i, d in enumerate(dates):
        ...
        try:
            ...
            # Guard: Tank01 returns fantasyPoints=0 for past slates.
            # If median proj is near-zero the data is useless for calibration.
            _proj_median = pool.loc[pool["proj"] > 0, "proj"].median()
            if pd.isna(_proj_median) or _proj_median < 3:
                results.append({"date": date_str, "status": "skipped",
                                "detail": "Tank01 returned zero projections (historical)"})
                progress.progress((i + 1) / len(dates))
                time.sleep(1)
                continue
