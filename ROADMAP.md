...
| 68 | **One-date one-click cascade** — "Fetch Pool from API" now auto-fetches actuals for past dates (stores in `st.session_state.actuals[date]`); Override slate date dropdown populated from `actuals` keys; Calibration Check wired to use API-fetched actuals + pool projections; Section C actuals date auto-defaults to pool date; specific error message when actuals API returns no data | `streamlit_app.py` | latest |
