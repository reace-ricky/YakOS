...
| 97 | **Fix 2: Dual Projection Uploads + proj_fp wired end-to-end** — (1) `_merge_external_proj(pool, source, rg_df, fp_df)` pure function computes `proj_fp` from the selected projection source (RotoGrinders / FantasyPros / Blended average); unmatched players fall back to base `proj`; (2) `_apply_proj_source(pool, source)` thin Streamlit wrapper reads `rg_proj_df` / `fp_proj_df` from session state; (3) Sidebar "Projection style" selectbox dynamically shows RotoGrinders / FantasyPros / Blended options based on uploaded files (disabled with placeholder when none uploaded); unique key `sidebar_proj_source` — no collision with `MIN_SALARY_USED` or other widgets; (4) `proj_fp` applied to `pool_df` on every rerun (before tabs render) so all tabs see the active projection; (5) Lab Tab: new "📊 Upload Projection Sources" section with separate RG + FP file uploaders, clear buttons, and load status captions; (6) Slate Room: High-Value Plays prefers `proj_fp` → `adjusted_proj` → `proj`; Player Projections table shows `proj_fp` as "Active Proj" first column, sorted/valued by `proj_fp`; (7) Optimizer: passes `proj_col="proj_fp"` when an external source is active, falls back to `opt_proj_style` (floor/ceil/sim85) otherwise; (8) Calibration: error diagnostics prefer `proj_fp` over `proj` for both the initial check and "Re-project & Compare"; (9) Sims: optimizer call passes `proj_col="proj_fp"` when active source is set; (10) 15 new unit tests in `tests/test_proj_source.py` covering all merge paths and edge cases. | `streamlit_app.py`, `tests/test_proj_source.py` | Fix 2 |
| 98 | **Wiring verification & gap fixes** — Identified and fixed four concrete wiring gaps found during end-to-end verification audit: (1) **CSV pool → historical actuals** (HIGH): Lab CSV upload now shows a "Slate date for this CSV" date input; when a past date is selected, auto-fetches actuals via Tank01 API (if key set) and stores `HistoricalSlateBundle` exactly as the API-fetch path does; graceful fallback when no API key or no games scheduled; `pool_date` session-state key set on every CSV load; (2) **Manual override validation** (MED): after parsing override text, names that don't match any player in the loaded pool now surface a `st.warning` banner listing the unmatched names, preventing silently-ignored typos; (3) **Alert validation projection alignment** (MED): `run_alert_backtest` is now called with `proj` overwritten from `proj_fp` when an external projection source is active in the sidebar, ensuring Alert Validation scores against the same projection column the sim engine uses; (4) **Dead-code imports removed** (LOW): `identify_calibration_gaps`, `action_queue_items`, `suggest_config_from_queue` removed from the `calibration` import block in `streamlit_app.py` (they remain exported from `yak_core/calibration.py`). | `streamlit_app.py` | #98 |
