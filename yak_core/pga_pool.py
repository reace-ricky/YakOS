...
def build_pga_pool(
    ...
    if "position" in pool.columns:
        pool = pool.drop(columns=["position"])
    pool["pos"] = "G"  # All golfers — no positional constraints in PGA DFS
    pool["team"] = ""  # No teams
    pool["opp"] = ""   # No opponents
    pool["sport"] = "PGA"

    # Drop any duplicate columns from multi-endpoint merges
    if pool.columns.duplicated().any():
        _dup = list(pool.columns[pool.columns.duplicated(keep=False)].unique())
        print(f"[pga_pool] Dropping duplicate columns: {_dup}")
        pool = pool.loc[:, ~pool.columns.duplicated(keep="last")]
